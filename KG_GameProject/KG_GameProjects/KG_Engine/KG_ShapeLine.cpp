...
HRESULT KG_ShapeLine::CreateInputLayout()
{
	...
	hr = m_obj.m_pd3dDevice->CreateInputLayout(layout, iElementCount, m_obj.m_pVertexCode->GetBufferPointer(), m_obj.m_pVertexCode->GetBufferSize(), m_obj.m_pVertexLayout.GetAddressOf());
	return hr;
}
...
HRESULT KG_ShapeLine::CreateVertexBuffer()
{
	...
	hr = m_obj.m_pd3dDevice->CreateBuffer(&pDesc,
		&pInitialData,
		m_obj.m_pVertexBuffer.GetAddressOf());
	return hr;
}
